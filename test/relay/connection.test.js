...
describe('relay', function () {
  describe('connection', function () {
      ...
      return this.sequelize.sync({force: true}).then(() => {
        return this.User.create({
          [this.User.Tasks.as]: [
            ...
            {name: Math.random().toString(), createdAt: new Date(Date.now() - 1500)}
          ]
        }, {
    ...
    it('should support in-query slicing and pagination with an orderBy', function () {
      ...
      return query().then((result) => {
        verify(result, firstThree);

        return query(result.data.user.tasks.edges[2].cursor).then((result) => {
          verify(result, nextThree);

          return query(result.data.user.tasks.edges[2].cursor).then((result) => {
            verify(result, lastThree);
          });
        });
      });
    });
